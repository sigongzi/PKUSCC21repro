...
prefix="${dataset}_${algorithm}_p${p_number}_weak"
...
if [ -f ${dataset_file} ];then
    ...
    echo $s
    ramble_weak_experiments.py --basedir ${compiledir}/ramBLe --scratch ${scratchdir} -g "-o ${scratchdir}/${prefix}.dot" --hostfile ${hostfile} --weak $n --mpi-arguments "-env MV2_CPU_MAPPING ${s}" -p ${p_number} -r 1 -a ${algorithm} -d ${dataset_file} -s $'\t' -c -v -i  --results ${output_file} > ${outputdir}/${prefix}.save 
else
