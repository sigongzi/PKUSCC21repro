...
prefix="${dataset}_${algorithm}_p${core_number}_strong"
...
if [ -f ${dataset_file} ];then
    ...
    echo $s
    ramble_experiments.py --basedir ${compiledir}/ramBLe --scratch ${scratchdir}/${prefix}.dot --mpi-arguments "-env MV2_CPU_MAPPING ${s}" -p ${core_number} -r 1 -a ${algorithm} -d ${dataset_file} -s $'\t' -c -v -i  --results ${output_file} > ${outputdir}/${prefix}.save 
else
